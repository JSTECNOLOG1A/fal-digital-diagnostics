# Prompt — entrega segura (Git + Deploy Método FAL)

Para quem usa **Claude Code** ou **Cursor**.  
Cole o bloco abaixo ao **terminar um ajuste** ou ao pedir deploy.

No Claude Code, o arquivo `CLAUDE.md` na raiz já lembra essas regras; ainda assim, cole o prompt na entrega para forçar a sequência completa.

---

```text
Você é o agente de entrega do Método FAL (repo fal-digital-diagnostics). Ferramenta: Claude Code ou Cursor.
...
## Regras absolutas
1. Nunca commitar ou enviar ao remoto: `.env`, `.env.local`, `.env.production`, `backend/.env`, senhas, tokens, chaves, dumps.
...
4. Nunca sobrescrever `/var/www/html/metodofal/.env.production` no VPS.
...
No VPS (`/var/www/html/metodofal`):
1. Sincronizar código do commit (`git pull` se clone; senão rsync/scp do tree commitado).
2. Rodar:
   `docker compose -f docker-compose.traefik.yml --env-file .env.production up -d --build`
3. Esperar `fal-prod-web` e `fal-prod-api` healthy.
4. Smoke:
...
1. Abre o projeto no Claude Code (lê `CLAUDE.md` automaticamente).
2. Faz o ajuste normalmente.
3. No fim, cola o prompt acima **ou** digita:
   - `Salvar no git, push e deploy no VPS. Ajuste: …`
4. Confirma ações destrutivas / SSH se o agente pedir.

## Exemplos

**Só salvar:**
> Salvar no git. Ajuste: corrige RBAC no cockpit.

**Salvar + remoto + produção:**
> [colar o prompt]  
> Salvar no git, push e deploy no VPS. Ajuste: tela de integrações.
