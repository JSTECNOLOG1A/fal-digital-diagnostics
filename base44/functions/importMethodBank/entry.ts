...
// Frozen global weights — import cannot change these
...
Deno.serve(async (req) => {
  try {
    ...
    if (!bank) return Response.json({ error: 'Campo "bank" é obrigatório no payload' }, { status: 400 });
    ...
    // Validate each dimension has at least 1 question
    ...
    // Validate each crossing has at least 1 MQE question
    ...
    // Build dimensions using FROZEN weights (import cannot override)
    const dimensions = REQUIRED_DIM_KEYS.map((key, idx) => {
      const d = bank.dimensions.find(x => x.key === key);
      return {
        key,
        name: d?.name || key,
        global_weight: FROZEN_WEIGHTS[key], // frozen — never from import
        order: idx + 1
      };
    });
    ...
    // Re-use penalty profiles from active method or use defaults
    ...
    // Create new MethodVersion (draft by default, unless activate=true)
    ...
    // Import questions per dimension
    const questionData = [];
    for (const dim of bank.dimensions) {
      ...
      dimQs.forEach((q, i) => {
        ...
        const weight = Math.min(3, Math.max(1, parseInt(q.weight) || 1));
        questionData.push({
    ...
    // Import MQE questions per crossing
    const mqeData = [];
    for (const cross of bank.crossings) {
      const cQs = cross.mqe_questions || [];
      cQs.forEach((q, i) => {
        const sectorTags = (q.sector_tags || ['all']).filter(t => VALID_SECTORS.includes(t));
        const sectorType = VALID_TYPES.includes(q.sector_type) ? q.sector_type : 'core';
        const weight = Math.min(3, Math.max(1, parseInt(q.weight) || 1));
    ...
    // Import checklist items per dimension
    const checklistData = [];
    for (const dim of bank.dimensions) {
      const items = dim.checklist || [];
    ...
    await base44.entities.AuditLog.create({
      ...
      action: 'method_bank_imported',
      entity_type: 'MethodVersion',
      entity_id: mv.id,
      details: { version_code, questions: questionData.length, mqe: mqeData.length, checklist: checklistData.length, activated: !!activate }
    });
    ...
    return Response.json({
      ...
      note: activate ? 'MethodVersion ativada. Novos tenants usarão esta versão.' : 'MethodVersion criada como draft. Ative manualmente quando pronto.'
    });
  } catch (error) {
