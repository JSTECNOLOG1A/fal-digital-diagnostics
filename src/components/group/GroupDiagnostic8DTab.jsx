/**
 * GroupDiagnostic8DTab — Aba Diagnóstico 8D do Grupo.
...
import {
  BarChart3, PlayCircle, ArrowRight, Clock, CheckCircle2,
  AlertCircle, Loader2, ChevronRight
} from 'lucide-react';
import { format } from 'date-fns';

// Rótulos amigáveis — sem labels técnicos como "scoring"
const STATUS_CONFIG = {
 ...
 * @param {any=} props.onGoToActionPlan
 */
export default function GroupDiagnostic8DTab({ groupId, tenantId, onGoToActionPlan }) {
  const navigate = useNavigate();
  const { assessment, assessments, loading } = useGroupAssessment(groupId, tenantId);

  const { data: snaps = [] } = useQuery({
    queryKey: ['8d-snap', assessment?.id, tenantId],
  ...
  const { data: plans = [] } = useQuery({
    queryKey: ['8d-plan', assessment?.id, tenantId],
  ...
  if (!assessment) {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-10 text-center">
        ...
        <h2 className="text-base font-bold text-slate-900 mb-2">Nenhum Diagnóstico 8D iniciado para este Grupo</h2>
        <p className="text-sm text-slate-500 max-w-md mx-auto mb-6">
          O Diagnóstico 8D é a leitura estrutural do Grupo nas oito dimensões FAL.
        </p>
        <Button
          onClick={() => navigate(createPageUrl(`FalAssessmentSetup?group_id=${groupId}`))}
          className="text-white gap-2"
          style={{ background: '#2563eb' }}
        >
          <PlayCircle className="w-4 h-4" /> Iniciar Diagnóstico 8D
        </Button>
      </div>
    );
  }
  ...
  // Sublabel contextual — sem "Em andamento 100%"
  const progressLabel =
  ...
  const btnLabel = isPublished ? 'Abrir Diagnóstico 8D' : isComplete ? 'Publicar Diagnóstico 8D' : 'Continuar Diagnóstico 8D';
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="px-5 py-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
            <div className="rounded-xl p-3 text-center" style={{ background: '#f8fafc', border: '1px solid #f1f5f9' }}>
              ...
              <p className="text-[10px] text-slate-500 mt-0.5">IFME™</p>
            </div>
            <div className="rounded-xl p-3 text-center" style={{ background: '#f8fafc', border: '1px solid #f1f5f9' }}>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {hasPlan && onGoToActionPlan && (
              <Button variant="outline" size="sm" onClick={onGoToActionPlan} className="gap-1.5">
      ...
      {others.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-slate-500 mb-2">Outros diagnósticos ativos deste grupo</p>
            ...
            {others.map(a => {
              ...
              return (
                <button
                  ...
                  style={/** @type {any} */ ({ '--hover-bg': '#f8fafc' })}
